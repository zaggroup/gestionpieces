// =====================================================
// ZAG GROUP — Service Worker v14.0
...
// ── INSTALL : mise en cache initiale (pour 1ere visite hors-ligne) ──
...
self.addEventListener('fetch', function(event) {
  ...
  // Polices Google → Cache-first avec fallback réseau (statiques, sans risque)
  if (isFont) {
    event.respondWith(
  ...
  // Fichiers app / navigation → NETWORK-FIRST
  // Toujours essayer le réseau (GitHub Pages = à jour immédiatement).
  // Le cache ne sert QUE si le réseau échoue (vraiment hors-ligne).
  if (isAppFile || event.request.mode === 'navigate') {
    event.respondWith(
      fetch(event.request).then(function(response) {
        // Met à jour le cache UNIQUEMENT si la réponse est valide (200)
        ...
        // Réseau indisponible → repli sur le cache (mode hors-ligne)
        return caches.match(event.request).then(function(cached) {
          return cached || new Response(
            '<h1>ZAG GROUP</h1><p>Hors-ligne — ouvrez l\'application une fois en ligne pour activer le mode hors-ligne.</p>',
